...
contract TakeProfitsHook is BaseHook, ERC1155 {
    ...
    // ===== State =====
    // Nested mapping to store pending orders
    // Can identify a position: pendingOrders[poolId][tickToSellAt][zeroForOne] = inputAmount
    mapping(PoolId poolId =>
        mapping(int24 tickToSellAt =>
            mapping(bool zeroForOne => uint256 inputAmount)))
                public pendingOrders;

    // Track total supply of claim tokens issued for each order position
    mapping(uint256 orderId => uint256 claimsSupply)
        public claimTokensSupply;

    // Track amount of output tokens claimable for each order position
    mapping(uint256 orderId => uint256 outputClaimable)
        public claimableOutputTokens;

    // Last known tick for each pool
    ...
    /// @param tickToSellAt The tick at which to sell (will be rounded down to usable tick)
    /// @param zeroForOne The direction of the swap
    /// @param inputAmount The amount of input tokens to place the order for
    /// @return tick The actual tick at which the order was placed
    function placeOrder(
        PoolKey calldata key,
        int24 tickToSellAt,
        bool zeroForOne,
    ...
    /// @param key The pool key
    /// @param tickToSellAt The tick at which the order was placed (will be rounded down to usable tick)
    /// @param zeroForOne The direction of the swap
    ...
    /// @notice Redeem output tokens from a filled order
    /// @param key The pool key
    /// @param tickToSellAt The tick at which the order was placed (will be rounded down to usable tick)
    /// @param zeroForOne The direction of the swap
    /// @param inputAmountToClaimFor The amount of input tokens (claim tokens) to claim output for
    function redeem(
        PoolKey calldata key,
        int24 tickToSellAt,
        bool zeroForOne,
    ...
    /// @notice Execute a pending order by conducting a swap
    /// @param key The pool key
    ...
    /// @notice Internal function to swap through the pool manager and settle all balances
    /// @param key The pool key
    ...
    /// @notice Internal function to settle tokens with the pool manager
    ...
    // ===== Helper Functions =====
    /// @notice Get a unique order ID from pool key, tick, and direction
    /// @param key The pool key
    /// @param tick The tick at which the order is placed
    /// @param zeroForOne The direction of the swap
    ...
    /// @notice Get the closest lower usable tick given an arbitrary tick value
    /// @dev Uniswap V4 pools can only conduct swaps at ticks that are multiples of tickSpacing.
    ///      E.g. if tickSpacing = 60, valid ticks are ..., -120, -60, 0, 60, 120, ...
    ///      This function rounds DOWN to the nearest usable tick.
    ///      Example: tickSpacing = 60, tick = 100 => returns 60
    ///      Example: tickSpacing = 60, tick = -100 => returns -120
    /// @param tick The arbitrary tick value
    /// @param tickSpacing The tick spacing of the pool
    /// @return The closest lower usable tick
